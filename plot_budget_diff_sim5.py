...
experiment_id_list = ['h2pert','nhh2pert','shh2pert','avih2pert','shiph2pert']

linestyle_list = {'h2pert':'-',
                  'nhh2pert':'--',
                  'shh2pert':'-.',
                  'avih2pert':':',
                  'shiph2pert':(0, (5, 2))}
...
for model_id in model_list:
    ...
    for experiment_id in experiment_id_list:
        ...
        file_path = (
            'annual_budget_csv/'
            + variable_id + '_' + model_id + '_' + member_id + '_'
            + project_id + '_' + experiment_id + '.csv'
        )

        if os.path.exists(file_path):
            budget_annual_pert = pd.read_csv(file_path, index_col=0)
            ...
            for c,col in enumerate(budget_annual_diff.columns):
                axs[c].plot(budget_annual_diff[col],linestyle = linestyle_list[experiment_id],
                            color=color_list[model_id],label=model_id+':'+experiment_id)
                axs[c].set_title(title_dict[col])

axs[0].legend()
plt.suptitle('SIM#5 experiments - cntr, budget diff for: ' + variable_id,fontsize=16)
plt.show()
